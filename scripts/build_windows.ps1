            ...
            try {
                ...
                if ($health.StatusCode -eq 200) {
                    ...
                    if ($env:OMEGA_VARIANT -ne "cpu") {
                        try {
                            $backend = Invoke-WebRequest -Uri "http://127.0.0.1:$smokePort/backend/snapshot" -UseBasicParsing -TimeoutSec 2
                            if ($backend.Content -notmatch '"gpu_acceleration_detected":\s*true') {
                                Write-Host "Smoke gate warning: GPU variant does not report GPU acceleration!"
                                $smokeOk = $false
                            }
                        } catch {
                            Write-Host "Smoke gate: Could not probe /backend/snapshot"
                            $smokeOk = $false
                        }
                    }
                    break
