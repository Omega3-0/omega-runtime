...
def get_omega_variant() -> str:
    """Resolve the build variant (cpu / cuda / vulkan / dml) from ``OMEGA_VARIANT``.

    Set by ``scripts/build_*.ps1`` and the variant matrix builds; consumed by
    ``_resolve_backend()`` and surfaced on ``/v1/version``. Defaults to ``cpu``
    when unset or set to anything outside the known matrix.
    """
    raw = (os.environ.get("OMEGA_VARIANT") or "").strip().lower()
